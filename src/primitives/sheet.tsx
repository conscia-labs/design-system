...
function Sheet({ ...props }: React.ComponentProps<typeof Drawer.Root>) { return <Drawer.Root data-slot="sheet" swipeDirection="right" {...props} />; }
...
const sideClasses = { top: "inset-x-0 top-0 max-h-[90vh] border-b", right: "right-0 top-0 h-full max-w-md border-l sm:max-w-lg", bottom: "inset-x-0 bottom-0 max-h-[90vh] border-t", left: "left-0 top-0 h-full max-w-md border-r sm:max-w-lg" };
...
function SheetContent({ className, children, side = "right", closeLabel = "Close", ...props }: React.ComponentProps<typeof Drawer.Popup> & { side?: keyof typeof sideClasses; closeLabel?: string }) {
  return <SheetPortal><SheetOverlay /><Drawer.Viewport className="fixed inset-0 z-50 pointer-events-none"><Drawer.Popup data-slot="sheet-content" className={cn("pointer-events-auto absolute flex w-full flex-col gap-6 bg-background p-6 text-foreground shadow-lg outline-none data-open:animate-in data-closed:animate-out", sideClasses[side], className)} {...props}>{children}<Drawer.Close data-slot="sheet-close" className="absolute right-4 top-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background [&_svg]:size-4"><XIcon /><span className="sr-only">{closeLabel}</span></Drawer.Close></Drawer.Popup></Drawer.Viewport></SheetPortal>;
}
...
function SheetDescription({ className, ...props }: React.ComponentProps<typeof Drawer.Description>) { return <Drawer.Description data-slot="sheet-description" className={cn("text-sm text-muted-foreground", className)} {...props} />; }
function SheetBody({ className, ...props }: React.ComponentProps<"div">) { return <div data-slot="sheet-body" className={cn("flex min-h-0 flex-1 flex-col gap-[var(--ds-space-group)] overflow-y-auto pr-1", className)} {...props} />; }
