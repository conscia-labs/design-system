...
type SearchableSelectProps = { id?: string; name?: string; value?: string; options: SearchableSelectOption[]; onValueChange: (value: string) => void; placeholder?: string; searchPlaceholder?: string; emptyMessage?: string; disabled?: boolean; clearable?: boolean; clearLabel?: string; optionsLabel?: string; className?: string; "aria-label"?: string };

function SearchableSelect({ id, name, value = "", options, onValueChange, placeholder = "Select an option", searchPlaceholder = "Search...", emptyMessage = "No matching options.", disabled = false, clearable = false, clearLabel = "Clear selection", optionsLabel, className, "aria-label": ariaLabel }: SearchableSelectProps) {
  ...
  return <div className={cn("relative min-w-0", className)}><Combobox.Root items={items} autoHighlight open={open} onOpenChange={(nextOpen) => { setOpen(nextOpen); if (!nextOpen) setInputValue(selected?.label ?? ""); }} value={value} inputValue={open ? inputValue : selected?.label ?? ""} onInputValueChange={setInputValue} onValueChange={(next) => { const nextOption = options.find((option) => option.value === next); setInputValue(nextOption?.label ?? ""); onValueChange(next ?? ""); }} itemToStringLabel={(item) => options.find((option) => option.value === item)?.label ?? ""} itemToStringValue={(item) => item ?? ""} name={name} disabled={disabled}>
    <div className="relative flex items-center">
      ...
      <Combobox.Trigger aria-label={ariaLabel ?? placeholder} className="absolute right-1 inline-flex size-8 items-center justify-center rounded-sm text-muted-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring"><ChevronsUpDown className="size-4" /></Combobox.Trigger>
    </div>
    <Combobox.Portal><Combobox.Positioner className="z-50 w-[var(--anchor-width)]"><Combobox.Popup className="mt-1 max-h-72 overflow-auto rounded-md border bg-popover p-1 text-popover-foreground shadow-[var(--ds-shadow-floating)] outline-none"><Combobox.List aria-label={optionsLabel ?? "Options"}><Combobox.Empty className="px-2 py-3 text-sm text-muted-foreground">{emptyMessage}</Combobox.Empty>{options.map((option) => <Combobox.Item key={option.value} value={option.value} disabled={option.disabled} className="ds-type-menu-item data-highlighted:bg-surface-muted relative flex cursor-default items-start gap-2 rounded-sm px-2 py-2 pr-8 outline-none data-disabled:pointer-events-none data-disabled:opacity-50"><span className="mt-0.5 flex size-4 shrink-0 items-center justify-center"><Combobox.ItemIndicator><Check className="size-4" /></Combobox.ItemIndicator></span><span className="min-w-0"><span className="block truncate text-sm">{option.label}</span>{option.description ? <span className="block truncate text-xs text-muted-foreground">{option.description}</span> : null}</span></Combobox.Item>)}</Combobox.List></Combobox.Popup></Combobox.Positioner></Combobox.Portal>
  </Combobox.Root></div>;
}
