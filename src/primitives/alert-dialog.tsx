...
function AlertDialogContent({ className, children, ...props }: React.ComponentProps<typeof BaseAlertDialog.Popup>) { return <AlertDialogPortal><AlertDialogOverlay /><BaseAlertDialog.Viewport className="fixed inset-0 z-50 flex items-center justify-center p-4"><BaseAlertDialog.Popup data-slot="alert-dialog-content" className={cn("relative grid w-full max-w-[calc(100%-2rem)] gap-5 rounded-lg border bg-canvas p-5 text-text-primary shadow-lg outline-none sm:max-w-lg sm:p-6", className)} {...props}>{children}<BaseAlertDialog.Close aria-label="Close" className="absolute right-4 top-4 rounded-xs opacity-70 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-focus [&_svg]:size-4"><XIcon /></BaseAlertDialog.Close></BaseAlertDialog.Popup></BaseAlertDialog.Viewport></AlertDialogPortal>; }
