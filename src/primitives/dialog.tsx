...
function DialogOverlay({ className, ...props }: React.ComponentProps<typeof BaseDialog.Backdrop>) { return <BaseDialog.Backdrop data-slot="dialog-overlay" className={cn("fixed inset-0", overlayLayers.modal, "bg-black/50 data-open:animate-in data-closed:animate-out", className)} {...props} />; }
function DialogContent({ className, children, showCloseButton = true, size = "default", closeLabel = "Close", ...props }: React.ComponentProps<typeof BaseDialog.Popup> & { showCloseButton?: boolean; size?: "small" | "default" | "large"; closeLabel?: string }) {
  return <DialogPortal><DialogOverlay /><BaseDialog.Viewport className={cn("fixed inset-0 flex items-center justify-center p-4", overlayLayers.modal)}><BaseDialog.Popup data-slot="dialog-content" data-size={size} className={cn("relative grid max-h-[min(42rem,calc(100vh-2rem))] w-full max-w-[calc(100%-2rem)] grid-rows-[auto_minmax(0,1fr)_auto] gap-5 overflow-hidden rounded-lg border bg-canvas p-5 text-text-primary shadow-lg outline-none sm:p-6", "data-[size=small]:max-w-sm data-[size=default]:max-w-lg data-[size=large]:max-w-3xl", className)} {...props}>{children}{showCloseButton ? <BaseDialog.Close data-slot="dialog-close" className="absolute right-4 top-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-focus focus:ring-offset-2 focus:ring-offset-canvas disabled:pointer-events-none [&_svg]:size-4"><XIcon /><span className="sr-only">{closeLabel}</span></BaseDialog.Close> : null}</BaseDialog.Popup></BaseDialog.Viewport></DialogPortal>;
}
